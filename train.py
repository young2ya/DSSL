...
def CNN_train(train_loader, model, optimizer, criterion):
    losses = AverageMeter()
...
def HCAE_train(train_loader, unlabel_loader, model, optimizer, criterion, u_criterion, epoch, args):
    losses = AverageMeter()
    acces = AverageMeter()
    correct = 0
    total = 0
    ...
    label_train_iter = iter(train_loader)
    unlabel_train_iter = iter(unlabel_loader)

    model.train()
    for batch_idx in range(args.train_iteration):
        try:
            inputs_x, targets_x = next(label_train_iter)
        except:
            label_train_iter = iter(train_loader)
            inputs_x, targets_x = next(label_train_iter)
        try:
            inputs_u,_ = next(unlabel_train_iter)
        except:
            unlabel_train_iter = iter(unlabel_loader)
            inputs_u,_ = next(unlabel_train_iter)
        ...
        recon_loss = u_criterion(inputs_u, outputs)

        logits,_ = model(inputs_x)
        logits = torch.softmax(logits.detach(), dim=-1)
        _, pseudo_labels = torch.max(logits.detach(), 1)

        cls_loss = criterion(logits, targets_x)
        ...
        total += targets_x.size(0)
        correct += (pseudo_labels == targets_x).sum().item()
        losses.update(loss.item(), inputs.size(0))
        acces.update(100 * correct / total, inputs_x.size(0))
...
def FixMatch_train(train_loader, unlabel_loader, model, optimizer, args):
    losses = AverageMeter()
    ...
    for batch_idx in range(args.train_iteration):
        ...
        _, pred_x = torch.max(logits_x, dim=-1)
        prec = acc(pred_x, targets_x)
