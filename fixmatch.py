...
#  train code
def FixMatch_train(train_loader, unlabel_loader, model, optimizer, scheduler, args):
    losses = AverageMeter()
    ...
    for batch_idx in range(args.train_iteration):
        ...
        logits_x = torch.softmax(logits_x.detach(), dim=-1)
    ...
    logger.info('train_loss : {:.4f}'.format(losses.avg))
    logger.info('train_acc : {:.4f}'.format(acc.compute()))
...
def main():
    ...
    for i in range(args.ex_epochs):
        ...
        for epoch in range(args.start_epoch, args.epochs):
            ...
            FixMatch_train(train_loader, unlabel_loader, model, optimizer, scheduler, args)
